...
public static class CSVLogger
{
    ...
    public static void LogGameData(float time, float distance, GameObject preview)
    {
        ...
        // 判斷 preview 是否啟動，並將其狀態寫入 CSV
        string previewStatus = preview != null && preview.activeInHierarchy ? "no_preview" : "preview";

        // 寫入資料行
        using (StreamWriter writer = new StreamWriter(filePath, true))
        {
            string currentTime = System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            writer.WriteLine($"{time}, {distance}, {currentTime}, {previewStatus}");
        }
