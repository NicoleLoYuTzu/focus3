...
public class AliceGameActivate : MonoBehaviour
{
    public GameObject potion; // 放大藥水物件
    public TextMeshProUGUI infoTextUI; // 顯示訊息的 UI
    public Animator aliceAnimator; // 愛麗絲的動畫控制器

    public XRBaseInteractable hintButton; // 提示按鈕
    public XRBaseInteractable endButton; // 結束按鈕
    ...
    private void OnHintHoverEnter(XRBaseInteractor interactor)
    {
    ...
    private IEnumerator ShowMessageThenGrow()
    {
        ...
        {
            aliceAnimator.SetTrigger("Grow"); // 觸發愛麗絲變大動畫
        }

        if (potion != null)
        {
            Destroy(potion, 2f); // 2 秒後銷毀藥水
        }
    }
